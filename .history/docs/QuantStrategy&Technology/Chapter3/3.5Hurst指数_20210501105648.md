...
2. 针对不同的子区间长度 $\mathrm{n}$, 计算$\mathrm{A}$个子区间的平均重标极差: $(\mathrm{R} / \mathrm{S})_{\mathrm{n}}=\frac{1}{\mathrm{~A}} \sum_{\mathrm{k}=1}^{\mathrm{A}}(\mathrm{R} / \mathrm{S})_{\mathrm{k}}$。其中每个子区间上重标极差的计算步驟：
